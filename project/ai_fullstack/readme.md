...
- 将 bot 的响应式业务剥离
- vercel ai-sdk/react
  AI 前端应用，next.js(react ssr框架)
  @ai-sdk 封装了chatbot,一键式快速开发AI应用

- chatbot UI 响应式和AI 业务剥离
  -mock.js 流式输出
  rawResponse 支持流式输出(面试的时候不用说)
...
- mockjs 搜索接口
  GET
  /api/search?keyword=编码(URI)
  中文这种非ASCII 字符 需要编码 encodeURI('url')
  - 字符串匹配
  - like 模糊查询
  - 基于大模型的语义搜索
    前端 -> vue react...
...
Retriever(检索) Agument Generation(生成)

1. llm 可以去检索一些文档(私有知识库)
